> ⚠️ **STALE — inherited from calcio. This repo is now "The Coupon" (a mid-build
> clone). Read `HANDOFF.md` and `STATUS.md` first; ignore the World-Cup content
> below until the Batch-6 rebrand replaces this file.**

# World Cup 2026 Prediction League — AGENTS.md

This file provides Codex with project-specific context and configuration for every session.
...
## Agent slash-command protocol

Canonical command workflows live in `docs/agent-commands/` so Claude Code,
Codex, and future agents all follow the same source of truth. Tool-specific
folders such as `.claude/commands/` are thin wrappers only.

When the user types one of these commands, read the matching file in
`docs/agent-commands/` and follow it literally:
...
Preferred rhythm for future batches:
...
Do not implement directly on `main`. If a session accidentally does so,
`/phase-closeout` may recover only when the user explicitly asks the agent to
"do it for me" or equivalent, and must stage only the current batch's files.

## Phase close-out protocol

Close-out is triggered by `/phase-closeout <ids>` (see `docs/agent-commands/phase-closeout.md`). Do not auto-run it at end-of-phase — wait for the slash command so the user can review the work first. The global `~/.Codex/AGENTS.md` defers to this same explicit-trigger rule.
...
- CI: GitHub Actions — token in `.env` as `GITHUB_TOKEN`. **Do not foreground-poll CI in a tight loop** — each iteration pollutes chat context. Pattern: one immediate check, then one or two follow-up checks via background shell polls spaced ~3 min apart, OR push and rely on the cached result endpoint at the end. Never write 10+ polling lines into the conversation.
