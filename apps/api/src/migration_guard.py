...
``.railway/railway.ts`` pins the service to a single replica. Raise it and two containers run
...
matches what ``.railway/railway.ts`` actually asks Railway for — including the multi-region case,
...
def declared_replica_count(environ: Mapping[str, str] | None = None) -> int:
    ...
    try:
        ...
        raise MigrationOnBootUnsafe(
            ...
            ".railway/railway.ts declares, or unset it."
        ) from None
    if count < 1:
        raise MigrationOnBootUnsafe(
...
def assert_single_replica(environ: Mapping[str, str] | None = None) -> None:
    ...
    raise MigrationOnBootUnsafe(
        ...
        "lock of its own.\n"
        "Either return .railway/railway.ts to a single replica, or move migrations out of the "
